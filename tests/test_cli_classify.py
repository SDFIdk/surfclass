...
def test_cli_classify_genericmodel(cli_runner, testmodel1_filepath, tmp_path):
    # Use the same model from testmodel1 but using the generic classification command
    classification_data_path = (
        Path("./tests/data/classification_data").absolute().resolve()
    )
    f1 = classification_data_path.joinpath("6171_727_amplitude.tif")
    ...
    args = (
        ...
        f"-f {f2} -f {f3} -f {f4} {testmodel1_filepath} {tmp_path}/classification.tif"
    )
    result = cli_runner.invoke(cli, args.split(" "), catch_exceptions=False)
    assert result.exit_code == 0

    # Everything should then be the same result as in testmodel1:
    outfile = tmp_path / "classification.tif"
    ds = gdal.Open(str(outfile))
    srcband = ds.GetRasterBand(1)
    nodata = srcband.GetNoDataValue()
